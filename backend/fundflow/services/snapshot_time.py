...
def latest_snapshot_time_from_source_rows(rows, fallback_now_local):
    """优先根据东财 f124 行情时间决定 --latest 的快照刻度。"""
    source_times = [
        parsed_time
        for row in rows
        if (parsed_time := parse_source_timestamp(row.get("source_timestamp"))) is not None
    ]
    if source_times:
        return align_to_latest_trading_slot(max(source_times))
